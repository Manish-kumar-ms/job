...
const JobTable = () => {
 ...
 useEffect(() => {
  const filteredJob = adminJobs?.filter((job) =>
    ...
    job?.company?.name.toLowerCase().includes(searchJobs.toLowerCase())
  ) || [];

  // Prevent re-setting state if it's the same data
  if (JSON.stringify(filteredJob) !== JSON.stringify(filterJob)) {
